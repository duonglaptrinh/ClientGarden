...
namespace WebGLSupport
{
    ...
    public class WebGLInput : MonoBehaviour, IComparable<WebGLInput>
    {
...
#endif
            // モバイルの入力対応
            if (Application.isMobilePlatform || WebGLAdapter.IsMobileDevice)
            {
        ...
        RectInt GetElemetRect()
        {
            ...
            // モバイルの場合、強制表示する
            if (showHtmlElement || Application.isMobilePlatform || WebGLAdapter.IsMobileDevice)
            {
                var x = (int)(rect.x);
        ...
        public void OnSelect()
        {
            ...
            // モバイルの場合、強制表示する
            var isHidden = !(showHtmlElement || Application.isMobilePlatform || WebGLAdapter.IsMobileDevice);
            id = WebGLInputPlugin.WebGLInputCreate(WebGLInput.CanvasId, rect.x, rect.y, rect.width, rect.height, fontSize, input.text, input.placeholder, input.lineType != LineType.SingleLine, isPassword, isHidden, Application.isMobilePlatform || WebGLAdapter.IsMobileDevice);
        ...
        {
            if (!Application.isMobilePlatform && !WebGLAdapter.IsMobileDevice) return;
            if (!instances.ContainsKey(id)) return;
