...
namespace Takasho.GLD.VGS
{
    ...
    public class HouseSelectMenu : MonoBehaviour
    {
        ...
        public void UpdateHouseMenu(HouseSO[] houseList, Action<HouseSO> onHouseSet)
        {
            _menuTitle.text = "建物選択メニュー";
            _description.text = "カテゴリーを選択してください";
            _returnButton.gameObject.SetActive(false);
            Debug.Log(houseList.Length);

            for (int i = 0; i < _cardList.Count; i++)
            {
                if (i < houseList.Length)
                {
                    _cardList[i].UpdateContent(houseList[i]);
                    ...
                    button.onClick.AddListener(() => onHouseSet(houseList[ii]));
                }
                else
                {
