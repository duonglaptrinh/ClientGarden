...
public class ChangeLightmap : MonoBehaviour
{
    ...
    [SerializeField] int[] renderInfoCopy;
    ...
    public void CopyRenderInfo()
    {
        List<int> list = new List<int>();
        List<Renderer> listRens = new List<Renderer>();
        foreach (var item in lightingScenariosData.rendererInfos)
        {
            list.Add(item.renderer.GetInstanceID());
            listRens.Add(item.renderer);
        }
        renderInfoCopy = list.ToArray();
        renderers = listRens.ToArray();
        SaveEditor(this);
    }

    public void PasteRenderInfo()
    {
    ...
    private void LoadLightProbes()
    {
        ...
        catch { DebugExtension.LogWarning("Warning, error when trying to load lightprobes for scenario "); }
    }
    ...
    private void ApplyRendererInfo(RendererInfo[] infos)
    {
        try
        {
            ...
            {
                var info = infos[i];
                renderers[i].lightmapIndex = infos[i].lightmapIndex;
                if (!renderers[i].isPartOfStaticBatch)
                {
                    renderers[i].lightmapScaleOffset = infos[i].lightmapOffsetScale;
                }
                if (renderers[i].isPartOfStaticBatch && verbose == true)
                {
                    DebugExtension.Log("Object " + renderers[i].gameObject.name + " is part of static batch, skipping lightmap offset and scale.");
                }
            }
        }
        ...
        {
            DebugExtension.LogError("Error in ApplyRendererInfo:" + e.GetType().ToString());
        }
    }
    ...
    public void GenerateLightmapInfoStore()
    {
        ...
        DebugExtension.Log("stored info for " + renderers.Length + " meshrenderers");
