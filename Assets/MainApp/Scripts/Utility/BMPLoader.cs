...
namespace B83.Image.BMP
{
    ...
    public class BMPLoader
    {
        ...
        public BMPImage LoadBMP(BinaryReader aReader)
        {
            ...
            {
                DebugExtension.LogError("Not a BMP file");
                return null;
            }
            if (!ReadInfoHeader(aReader, ref bmp.info))
            {
                DebugExtension.LogError("Unsupported header format");
                return null;
            }
            ...
            {
                DebugExtension.LogError("Unsupported image format: "+ bmp.info.compressionMethod);
                return null;
            }
            ...
            {
                DebugExtension.LogError("Unsupported file format: " + bmp.info.compressionMethod + " BPP: " + bmp.info.nBitsPerPixel);
                return null;
            }
            return bmp;
        }
        ...
        private static void Read32BitImage(BinaryReader aReader, BMPImage bmp)
        {
            ...
            {
                DebugExtension.LogError("Unexpected end of file.");
                return;
            }
        ...
        private static void Read24BitImage(BinaryReader aReader, BMPImage bmp)
        {
            ...
            {
                DebugExtension.LogError("Unexpected end of file. (Have "+ (aReader.BaseStream.Position + count)+" bytes, expected " + aReader.BaseStream.Length+" bytes)");
                return;
            }
            int shiftR = GetShiftCount(bmp.rMask);
            int shiftG = GetShiftCount(bmp.gMask);
        ...
        private static void ReadIndexedImage(BinaryReader aReader, BMPImage bmp)
        {
            ...
            for (int y = 0; y < h; y++)
            {
                for (int x = 0; x < w; x++)
                {
                    ...
                    {
                        DebugExtension.LogError("Indexed bitmap has indices greater than it's color palette");
                        return;
                    }
